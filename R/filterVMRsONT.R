...
#'   or "CHH").
#' @param test Statistical test for VMR calling ("fisher" or "score").
...
filterVMRsONT <- function(methylationData1, 
  ...
  if (!is.null(cores)){
    .stopIfNotAll(c(.isInteger(cores, positive=TRUE)), 
                  " the number of cores to used when computing the VMRs needs to be an integer  hirger or equal to 1.")
  ...
  if (!is.null(varRatioFc)) {
    ...
    "‘varRatioFc’ must be a single numeric value ≥ 1 (e.g. 2 for a 2× variance cutoff)")
  }
  ...
  if(length(potentialVMRs) > 0){
    ...
    if(length(computedVMRs) > 0){
      ...
      computedVMRs$wilcox_pvalue <- p.adjust(computedVMRs$wilcox_pvalue, method = "fdr")
      computedVMRs$f_pvalue <- p.adjust(computedVMRs$f_pvalue, method = "fdr")
