...
syntheticMethylationData1$readsM <- round(syntheticMethylationData1$readsN*proportion1)
...
test_filterDMRs <- function() {
  ...
  checkTrue(all(overlapsAny(DMRs_CG_noise_filter_filtered, DMRs[2:4])))
  checkTrue(all(overlapsAny(DMRs[2:4], DMRs_CG_noise_filter_filtered)))
}
...
test_computeDMRsReplicates <- function(){
  DMRsReplicatesNeighbourhood <- computeDMRsReplicates(methylationData = syntheticDataReplicates,   ### CHANGE; methylationData -> syntheticDataReplicates
                                                       condition = condition,
                                                       regions = NULL,
                                                       context = "CHH",
  ...
  checkTrue(overlapsAny(ranges(DMRsReplicatesNeighbourhood),
                        ranges(syntheticDataReplicates)))
